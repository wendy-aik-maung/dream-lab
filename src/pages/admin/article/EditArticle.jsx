...
import { useEditArticle,useGetSingleArticleByAdmin } from "../../../hooks/useArticles";
...
const EditArticle = () => {
  ...
  } = useGetSingleArticleByAdmin(slug);
  ...
  useEffect(() => {
    if (isSuccess) {
      setValue("articleCover", article?.mainImage);
      setValue("articlename", article?.title);
      setValue("duration", article?.readingTime);
      setValue("description", article?.shortDesc);
      setValue("content",article?.content)
      setIsFree(article?.isFree);
      setActive(article?.status === "a");

      setDefaultTitle(article.title);

      const defaultCategories = article?.categories.map((category) => {
        return { label: category.name, value: category.id };
      });
      setSelectedCategories(defaultCategories);

      const defaultAuthors = article?.articleAuthors.map((author) => {
        return { label: author.name, value: author.id };
      });
      setSelectedAuthors(defaultAuthors);
    }
  }, [isSuccess, setValue, article]);
	...
	const onSubmit = (data) => {
      ...
      defaultTitle,
		...
		articleEditMutation.mutate(submitData);
	};
	...
	return (
		<div className="container mx-auto">
			<header className=" flex gap-12  font-poppins font-bold text-xl mb-8">
				...
				<div>
					<h2 className="text-textColor1">Edit Articles</h2>
				</div>
			</header>
			{/* Form */}
					...
					<div className="basis-1/2">
                      ...
                      src={
                        typeof articleCover  === "string"
                          ? articleCover
                          : URL.createObjectURL(articleCover?.[0])
                      }
            ...
            ">
				...
				<div className="mt-[7rem] flex justify-center items-center gap-8">
					...
					<button
						...
						disabled={articleEditMutation.isLoading}>
						{articleEditMutation.isLoading ? (
